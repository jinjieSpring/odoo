...
class HdaiSession(models.Model):
    ...
    def action_execute_tool(self, payload):
        """Validate and execute a tool payload; returns an Odoo action or a
        friendly notification when the tool cannot run. ``BaseTool``
        registry entries keep their action semantics (e.g. open a view);
        ``hdai.tool`` registry entries (search tools, suggestion tools) are
        executed through the audited read-only/suggestive framework."""
            ...
            if 'suggestion_preview' in result:
                ...
                return {
                    ...
                    'params': {
                        ...
                        'message': _(
                            'Proposed update for %s #%s: %s. Reason: %s') % (
                            preview.get('model'),
                            preview.get('record_id'),
                            fields_text or '-',
    ...
    def _execute_loop_call(self, tool_name, arguments):
        ...
        if tool.suggestive:
            return (
                self._build_loop_card(tool, arguments, 'ready'),
                'suggestive', None)
        result = self.env['hdai.tool'].action_invoke_tool(
