# -*- coding: utf-8 -*-
"""Extensible tool-calling framework for Linkin AI.

Tools turn structured payloads emitted by the LLM into executable Odoo
actions. A tool is a ``BaseTool`` subclass registered in the module-level
registry. The chat pipeline only hands validated *tool cards* to the client
(never raw JSON); execution re-validates on the server at click time, and
blocked tools carry a user-facing reason plus an optional admin hint.

New tools only need to subclass ``BaseTool``, implement ``validate`` and
``execute``, and call ``register_tool`` at import time.
"""
...
def register_tool(tool):
    """Register a tool class (instance) under its ``name``."""
