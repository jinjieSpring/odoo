...
class LLMService:
    ...
    def stream_chat(model, messages, options=None):
        ...
        or ``error`` dicts. Never raises."""
        options = options or {}
        provider = model.provider_id
        messages = LLMService._with_system_instruction(messages, model, options)
        try:
            if provider.provider_type == 'ollama':
                yield from LLMService._stream_ollama(model, messages, options)
            elif LLMService._uses_responses_api(model, options):
                yield from LLMService._stream_responses_api(model, messages, options)
            else:
                yield from LLMService._stream_openai_compatible(
