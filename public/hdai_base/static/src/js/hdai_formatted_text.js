 ...
 */
export class HdaiFormattedText extends Component {
    static template = "hdai_base.FormattedText";
    static props = {
        content: { type: String },
    ...
    _processCitations(root) {
        for (const sup of root.querySelectorAll(".o_hdai_citation")) {
    ...
    _processBlocks(root) {
        for (const blockEl of root.querySelectorAll(
            ...
            wrapper.className = `o_hdai_block o_hdai_block_${type}`;
            const head = document.createElement("div");
            head.className = "o_hdai_block_head";
            const label = document.createElement("span");
            label.className = "o_hdai_block_label";
            label.textContent =
                type === "code" ? (lang || "code") : type;
            const actions = document.createElement("div");
            actions.className = "o_hdai_block_actions";
            const extension =
            ...
            body.className = "o_hdai_block_body";
    ...
    _actionButton(icon, title, onClick) {
        ...
        button.type = "button";
        button.className = "btn btn-link p-1 o_hdai_block_action";
        button.title = title;
    ...
    _renderCode(body, content, lang) {
        ...
        if (window.Prism && lang && window.Prism.languages[lang]) {
            try {
                ...
                console.error("hdai: prism highlight failed", error);
            }
        }
    }
    ...
    _renderSvg(body, content) {
        ...
        holder.className = "o_hdai_svg_holder";
    ...
    async _renderMermaid(body, content) {
        ...
        try {
            ...
            const holder = document.createElement("div");
            holder.className = "o_hdai_mermaid_svg";
            holder.innerHTML = svg;
            body.appendChild(holder);
        } catch (error) {
            console.error("hdai: mermaid render failed", error);
            fallback();
        }
    }
