 ...
 * placeholders that the HdaiFormattedText component turns into rendered
...
function inlineFormat(text) {
    ...
    out = out.replace(
        /\[SOURCE:(\d+)\]/g,
        '<sup class="o_hdai_citation" data-chunk-id="$1">[$1]</sup>'
    );
    out = out.replace(
        /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,
...
function makePlaceholder(blockType, content, lang) {
    ...
    return (
        `<div class="o_hdai_block o_hdai_${blockType}" ` +
