...
import { HdaiChat } from "@hdai_base/js/hdai_chat";
...
async function mountChat() {
    ...
    await mountWithCleanup(HdaiChat);
}

function typeInput(value) {
    const input = queryFirst(".o_hdai_chat_input");
    input.value = value;
...
test("user and assistant messages are rendered from the session", async () => {
    await mountChat();
    await waitFor(".o_hdai_chat_msg.o_hdai_user");
    expect(queryAll(".o_hdai_chat_msg.o_hdai_user")).toHaveCount(2);
    expect(queryAll(".o_hdai_chat_msg.o_hdai_assistant")).toHaveCount(2);
    expect(queryAll(".o_hdai_chat_msg_content")[0].textContent).toInclude(
        "first question"
    );
});
...
test("status bar shows formatted usage", async () => {
    await mountChat();
    await waitFor(".o_hdai_chat_status_tokens");
    expect(queryFirst(".o_hdai_chat_status_tokens").textContent).toBe(
...
test("session row tooltip carries stats and update time", async () => {
    await mountChat();
    const title = queryFirst(".o_hdai_session_item").getAttribute("title");
...
test("slash command palette filters and Tab completes", async () => {
    ...
    typeInput("/");
    await animationFrame();
    await waitFor(".o_hdai_chat_command");
    typeInput("/exp");
    await animationFrame();
    await waitFor(".o_hdai_chat_command");
    expect(queryAll(".o_hdai_chat_command")).toHaveCount(1);
    expect(queryFirst(".o_hdai_chat_command").textContent).toInclude("/export");
    keyOnInput("Tab");
    await animationFrame();
    const input = queryFirst(".o_hdai_chat_input");
...
test("editing a user message submits on plain Enter", async () => {
    await mountChat();
    await click(
        ".o_hdai_chat_msg.o_hdai_user .o_hdai_chat_msg_actions button:has(.fa-pencil)"
    );
    await waitFor(".o_hdai_chat_edit_msg textarea");
    const textarea = queryFirst(".o_hdai_chat_edit_msg textarea");
    textarea.focus();
    textarea.dispatchEvent(
        new KeyboardEvent("keydown", { key: "Enter", bubbles: true })
    );
    await waitForNone(".o_hdai_chat_edit_msg");
    expect(
        ormCalls.some(
...
test("regenerate and resend call the expected model actions", async () => {
    await mountChat();
    await click(
        ".o_hdai_chat_msg.o_hdai_assistant .o_hdai_chat_msg_actions button:has(.fa-rotate-right)"
    );
    await click(
        ".o_hdai_chat_msg.o_hdai_user .o_hdai_chat_msg_actions button:has(.fa-rotate-left)"
    );
    expect(
