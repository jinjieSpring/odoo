...
import { AiChatDialog } from "./ai_chat_dialog";
    ...
    async openChat(pendingMessage) {
        ...
        if (pendingMessage) {
            try {
                ...
                console.error("ai_base: failed to create session", error);
...
registry.category("actions").add("ai_refresh_systray", () => {
    window.dispatchEvent(new CustomEvent("ai:systray-refresh"));
    return { type: "ir.actions.client", tag: "soft_reload" };
});

// Open the AI Assistant tab of the settings page and refresh the systray icon.
registry.category("actions").add("ai_open_settings", () => {
    window.dispatchEvent(new CustomEvent("ai:systray-refresh"));
    return {
        type: "ir.actions.act_window",
        res_model: "res.config.settings",
...
registry.category("actions").add("ai_open_chat", (env) => {
    return (async () => {
        ...
        env.services.dialog.add(AiChatDialog, {
            sessionId: false,
